...
fn walk_trail(map: &Map, location: &Location) -> HashSet<Pos> {
    ...
    for (i_offset, j_offset) in [(0, 1), (0, -1), (1, 0), (-1, 0)] {
        ...
        if new_height == 9 {
            reachable_peaks.insert(new_pos);
            continue;
        }

        let mut already_visited = location.already_visited.clone();
        already_visited.insert(location.pos);
        ...
        reachable_peaks.extend(walk_trail(map, &new_location));
    }
